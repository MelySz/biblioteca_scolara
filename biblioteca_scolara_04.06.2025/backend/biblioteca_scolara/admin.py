...
# Exemplar inline pentru afișare în pagina cărții
class ExemplarInline(admin.TabularInline):
    model = Exemplar
    extra = 0
    fields = ['cod_unic', 'locatie', 'stare']
    show_change_link = True

# Admin pentru Cărți
class CarteAdmin(admin.ModelAdmin):
    list_display = ('titlu', 'autor', 'categorie', 'editura', 'exemplare_disponibile')
...
class ExemplarAdmin(admin.ModelAdmin):
    list_display = ('cod_unic', 'carte', 'isbn_carte', 'an_publicatie_carte', 'stare', 'locatie')
    search_fields = ('cod_unic', 'carte__titlu',)
    list_filter = ('stare', 'locatie', 'carte')

    def isbn_carte(self, obj):
        return obj.carte.isbn
    isbn_carte.short_description = "ISBN"

    def an_publicatie_carte(self, obj):
        return obj.carte.an_publicatie
    an_publicatie_carte.short_description = "An publicație"
