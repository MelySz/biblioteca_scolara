...
class ListaEleviView(APIView):
    ...
    def get(self, request):
        ...
        suspendati = request.GET.get("suspendati") == "true"
        doar_azi = request.GET.get("azi") == "true"
        luna = int(request.GET.get("luna", 0))
        an = int(request.GET.get("an", 0))
        azi = now().date()

        elevi = Utilizator.objects.filter(tip="elev")

        if clasa:
            elevi = elevi.filter(clasa=clasa)

        #dacă se cere filtrul „azi” sau „suspendati”
        if doar_azi or suspendati:
            elevi = elevi.filter(is_active=False)

        #doar elevii suspendați azi
        if doar_azi:
            elevi_azi = []
            for u in elevi:
                if u.data_suspendare and u.zile_suspendare:
                    delta = (azi - u.data_suspendare).days
                    ramase = u.zile_suspendare - delta
                    if ramase > 0:
                        u.zile_ramase = ramase
                        elevi_azi.append(u)
            elevi = elevi_azi

        #doar elevii suspendați într-o anumită lună/an
        elif suspendati and luna and an:
            filtrati = []
            for u in elevi:
                if u.data_suspendare and u.data_suspendare.month == luna and u.data_suspendare.year == an:
                    filtrati.append(u)
            elevi = filtrati
...
# Exemplare extinse - pentru rapoarte 
class ExemplareExtinseView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        date_raspuns = []

        exemplare = Exemplar.objects.select_related("carte")

        for exemplar in exemplare:
            try:
                imprumut = (
                    Imprumut.objects.filter(exemplar=exemplar, data_restituire__isnull=True)
                    .order_by("-data_imprumut")
                    .first()
                )

                titlu = exemplar.carte.titlu if exemplar.carte else "Fără titlu"
                autor = exemplar.carte.autor if exemplar.carte else "Fără autor"
                cod_unic = exemplar.cod_unic
                stare = exemplar.stare
                imprumutat = imprumut is not None
                ...
                if imprumut:
                    ...
                    zile_imprumut = imprumut.zile_imprumut
                    termen = imprumut.data_imprumut + timedelta(days=zile_imprumut)
                    data_retur = termen.strftime("%d.%m.%Y")
                    zile_ramase = max(0, (termen - date.today()).days)

                    elev_obj = getattr(imprumut, "utilizator", None)
                    if elev_obj:
                        elev = f"{elev_obj.nume} {elev_obj.prenume}"
                ...
                date_raspuns.append({
                    ...
                    "zile_imprumut": zile_imprumut,
                    "zile_ramase": zile_ramase,
                    "elev": elev
                })

            except Exception as e:
                print(f"Eroare la procesarea exemplarului {exemplar.id}: {e}")
                continue

        return Response(date_raspuns)
...
# Rapoarte împrumuturi și returnări
class RapoarteImprumuturiView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        tip = request.GET.get("tip", "imprumuturi")
        anul = int(request.GET.get("an", datetime.now().year))
        luna = int(request.GET.get("luna", 0))  # 0 înseamnă toate lunile

        if tip == "imprumuturi":
            queryset = Imprumut.objects.filter(data_imprumut__year=anul)
            if luna != 0:
                queryset = queryset.filter(data_imprumut__month=luna)
        else:
            queryset = Imprumut.objects.filter(returnat=True, data_restituire__year=anul)
            if luna != 0:
                queryset = queryset.filter(data_restituire__month=luna)

        queryset = queryset.select_related("utilizator", "exemplar__carte")

        rezultate = []
        for imprumut in queryset:
            rezultate.append({
                "cod_unic": imprumut.exemplar.cod_unic,
                "titlu": imprumut.exemplar.carte.titlu,
                "autor": imprumut.exemplar.carte.autor,
                "elev": f"{imprumut.utilizator.nume} {imprumut.utilizator.prenume}",
                "clasa": imprumut.utilizator.clasa,
                "data": imprumut.data_imprumut.strftime("%d.%m.%Y") if tip == "imprumuturi" else imprumut.data_restituire.strftime("%d.%m.%Y"),
            })

        return Response(rezultate)
    ...
    def post(self, request, pk):
        try:
            ...
            if imprumut.data_scadenta and imprumut.data_restituire.date() > imprumut.data_scadenta:
                ...
                elev.data_suspendare = now().date()
                elev.zile_suspendare = 3
                elev.save()
...
class RecenzieListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if respinse_param == "true":
            # Recenzii respinse (aprobat=False și respins=True)
            return Recenzie.objects.filter(aprobat=False, respins=True, comentariu__isnull=False)
        elif aprobat_param == "false":      
            # Recenzii în curs de aprobare (aprobat=False și respins=False)
            return Recenzie.objects.filter(aprobat=False, respins=False)
        else:
            # Recenzii aprobate
            return Recenzie.objects.filter(aprobat=True)
    ...
    def perform_create(self, serializer):
        user = self.request.user
        if user.tip != "elev":
            raise PermissionDenied("Doar elevii pot scrie recenzii.")
        serializer.save(utilizator=user, aprobat=False, respins=False)
...
# Respingere recenzie
class RespingereRecenzieView(generics.UpdateAPIView):
    permission_classes = [IsAuthenticated]
    queryset = Recenzie.objects.all()
    serializer_class = RecenzieSerializer
    ...
    def update(self, request, *args, **kwargs):
        ...
        if user.tip != "bibliotecar":
            return JsonResponse({"message": "Doar bibliotecarii pot respinge recenzii."}, status=403)

        recenzie.aprobat = False
        recenzie.respins = True
        recenzie.save()

        # Log
        Log.objects.create(
            utilizator=user,
            actiune=f"A respins recenzia pentru cartea {recenzie.carte.titlu}"
        )

        return JsonResponse({"message": "Recenzia a fost respinsă!"}, status=200)
    
# Stergere recenzie
class StergereRecenzieView(DestroyAPIView):
    queryset = Recenzie.objects.all()
    permission_classes = [IsAuthenticated]
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if user.tip == "bibliotecar":
            # Bibliotecarul poate șterge orice recenzie
            return super().delete(request, *args, **kwargs)
        elif user == recenzie.utilizator:
            # Utilizatorul poate șterge doar propriile recenzii
            return super().delete(request, *args, **kwargs)
        else:
            raise PermissionDenied("Nu ai permisiunea să ștergi această recenzie.")

# Recenziile mele aprobate
class RecenziileMeleAprobateView(ListAPIView):
    """Returnează recenziile aprobate ale elevului logat"""
    serializer_class = RecenzieSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        utilizator = self.request.user
        return Recenzie.objects.filter(utilizator=utilizator, aprobat=True)

# Recenziile mele in așteptare
class RecenziileMeleInAsteptareView(ListAPIView):
    """Recenziile în curs de aprobare ale elevului logat"""
    serializer_class = RecenzieSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        utilizator = self.request.user
        return Recenzie.objects.filter(utilizator=utilizator, aprobat=False, respins=False)

# Recenziile mele respinse
class RecenziileMeleRespinseView(ListAPIView):
    """Recenziile respinse ale elevului logat"""
    serializer_class = RecenzieSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        utilizator = self.request.user
        return Recenzie.objects.filter(utilizator=utilizator, aprobat=False, respins=True)



# Listă toate recenziile - fără filtrare
class RecenzieListAllView(ListAPIView):
    queryset = Recenzie.objects.all()
    serializer_class = RecenzieSerializer
    permission_classes = [IsAuthenticated]

# Export utilizatori Excel
class ExportUtilizatoriExcelView(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request):
        ...
        utilizatori = Utilizator.objects.filter(tip="elev")
        date_export = []

        if clasa:
            utilizatori = utilizatori.filter(clasa__startswith=clasa)

        if doar_azi:
            antet = ["ID", "Nume", "Prenume", "Clasa", "Zile rămase"]
            utilizatori = utilizatori.filter(is_active=False)
            ...
            for u in utilizatori:
                if u.data_suspendare and u.zile_suspendare:
                    ...
                    ramase = u.zile_suspendare - delta
                    if ramase > 0:
                        linie = [u.id, u.nume, u.prenume, u.clasa, ramase]
                        date_export.append(linie)

        elif suspendati:
            antet = ["ID", "Nume", "Prenume", "Clasa"]
            utilizatori = utilizatori.filter(is_active=False)

            if luna and an and luna != "0":
                utilizatori = utilizatori.filter(
                    data_suspendare__month=int(luna),
                    data_suspendare__year=int(an)
                )

            for u in utilizatori:
                linie = [u.id, u.nume, u.prenume, u.clasa]
                date_export.append(linie)
        ...
        wb = Workbook()
        ws = wb.active
        ws.title = "Utilizatori"
        ws.append(antet)

        for linie in date_export:
            ws.append(linie)

        response = HttpResponse(
            content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
        response["Content-Disposition"] = 'attachment; filename="rapoarte_utilizatori.xlsx"'
        wb.save(response)
        return response

# Export exemplare Excel
class ExportExemplareExcelView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        tip = request.GET.get("tip", "toate")  # toate | imprumutate | neimprumutate
        exemplare = Exemplar.objects.select_related("carte")

        if tip == "imprumutate":
            exemplare = exemplare.filter(stare="imprumutat")
        elif tip == "neimprumutate":
            exemplare = exemplare.exclude(stare="imprumutat")

        data_export = []

        for ex in exemplare:
            rand = {
                "Cod exemplar": ex.cod_unic,
                "Titlu": ex.carte.titlu if ex.carte else "",
                "Autor": ex.carte.autor if ex.carte else "",
            }

            if tip == "toate":
                rand["Disponibilitate"] = "Indisponibil" if ex.stare == "imprumutat" else "Disponibil"
            elif tip == "imprumutate":
                imprumut = Imprumut.objects.filter(exemplar=ex, returnat=False).first()
                if imprumut:
                    data_retur = imprumut.data_imprumut + timedelta(days=imprumut.zile_imprumut)
                    zile_ramase = max(0, (data_retur - date.today()).days)
                    rand.update({
                        "Elev": f"{imprumut.utilizator.nume} {imprumut.utilizator.prenume}",
                        "Data împrumut": imprumut.data_imprumut.strftime("%d.%m.%Y"),
                        "Data retur estimat": data_retur.strftime("%d.%m.%Y"),
                        "Zile rămase": zile_ramase
                    })

            data_export.append(rand)

        df = pd.DataFrame(data_export)

        response = HttpResponse(content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        response["Content-Disposition"] = f'attachment; filename="rapoarte_exemplare_{tip}.xlsx"'

        with pd.ExcelWriter(response, engine="openpyxl") as writer:
            df.to_excel(writer, index=False, sheet_name="Exemplare")

        return response

#Export împrumuturi și retururi Excel
class ExportImprumuturiRetururiView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        tip = request.GET.get("tip", "imprumuturi")
        an = int(request.GET.get("an", datetime.now().year))
        luna = int(request.GET.get("luna", 0))

        if tip == "imprumuturi":
            queryset = Imprumut.objects.filter(data_imprumut__year=an)
            if luna != 0:
                queryset = queryset.filter(data_imprumut__month=luna)
        else:
            queryset = Imprumut.objects.filter(returnat=True, data_restituire__year=an)
            if luna != 0:
                queryset = queryset.filter(data_restituire__month=luna)
        ...
        # Antet
        ws.append(["Cod exemplar", "Titlu", "Autor", "Elev", "Clasa", "Data"])

        for imprumut in queryset:
            data = imprumut.data_imprumut if tip == "imprumuturi" else imprumut.data_restituire
            ws.append([
                imprumut.exemplar.cod_unic,
                imprumut.exemplar.carte.titlu,
                imprumut.exemplar.carte.autor,
                f"{imprumut.utilizator.nume} {imprumut.utilizator.prenume}",
                imprumut.utilizator.clasa,
                data.strftime("%d.%m.%Y") if data else "-"
            ])

        response = HttpResponse(content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        response["Content-Disposition"] = f'attachment; filename=rapoarte_{tip}_{luna}_{an}.xlsx'
        wb.save(response)
        return response

# Export recenzii Excel
class ExportRecenziiExcelView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        import openpyxl
        from django.http import HttpResponse
        from biblioteca_scolara.models import Recenzie
        from django.utils.timezone import datetime

        tip = request.GET.get("tip", "toate")
        luna = int(request.GET.get("luna", 0))
        an = int(request.GET.get("an", datetime.now().year))

        recenzii = Recenzie.objects.all()

        if tip == "aprobate":
            recenzii = recenzii.filter(aprobat=True)
        elif tip == "neaprobate":
            recenzii = recenzii.filter(aprobat=False)

        if luna:
            recenzii = recenzii.filter(data_postare__month=luna)
        if an:
            recenzii = recenzii.filter(data_postare__year=an)

        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = "Recenzii"

        # Titlul coloanei de stare
        if tip == "aprobate":
            status_col = "Aprobat"
        elif tip == "neaprobate":
            status_col = "Neaprobat"
        else:
            status_col = "Aprobat / Neaprobat"

        ws.append(["Recenzie", "Elev", "Clasa", "Cartea", "Data", status_col])

        for r in recenzii:
            stare = "✔️" if r.aprobat else "❌"
            ws.append([
                r.comentariu or "-",
                f"{r.utilizator.nume} {r.utilizator.prenume}",
                r.utilizator.clasa or "-",
                r.carte.titlu,
                r.data_postare.strftime("%d.%m.%Y"),
                stare
            ])

        response = HttpResponse(content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        response["Content-Disposition"] = 'attachment; filename=\"rapoarte_recenzii.xlsx\"'
        wb.save(response)
        return response
