...
from biblioteca_scolara.serializers import (
    UtilizatorSerializer, CarteSerializer, ImprumutSerializer, ExemplarSerializer, RecenzieSerializer, LogSerializer
)
...
# Resetare parolă
class ResetareParolaView(APIView):
    ...
    def post(self, request):
        try:
            email = request.data.get("email")
            parola_noua = request.data.get("parola_noua")
            ...
            return Response({"message": "Parola a fost resetată cu succes."}, status=status.HTTP_200_OK)

        except json.JSONDecodeError:
            return Response({"message": "Format JSON invalid"}, status=status.HTTP_400_BAD_REQUEST)
