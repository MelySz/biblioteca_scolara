...
class ExemplarSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id',
            'titlu_carte', 'autor_carte', 'editura_carte', 'categorie_carte',
            'isbn', 'an_publicatie',
            'cod_unic', 'locatie', 'stare'
        ]
...
class ImprumutSerializer(serializers.ModelSerializer):
    carte_id = serializers.IntegerField(source='exemplar.carte.id', read_only=True)
    ...
    editura = serializers.CharField(source='exemplar.carte.editura', read_only=True)
    categorie = serializers.CharField(source='exemplar.carte.categorie', read_only=True)
    isbn = serializers.CharField(source='exemplar.carte.isbn', read_only=True)
    an_publicatie = serializers.IntegerField(source='exemplar.carte.an_publicatie', read_only=True)

    exemplar_cod = serializers.CharField(source='exemplar.cod_unic', read_only=True)
    locatie = serializers.CharField(source='exemplar.locatie', read_only=True)
    stare = serializers.CharField(source='exemplar.stare', read_only=True)
    ...
    data_restituire = serializers.SerializerMethodField()

    class Meta:
        model = Imprumut
        fields = [
            'id',
            'carte_id', 'carte', 'autor', 'editura', 'categorie', 'isbn', 'an_publicatie',
            'exemplar_cod', 'locatie', 'stare',
            'utilizator_nume', 'utilizator_prenume', 'utilizator_clasa',
            'data_imprumut', 'data_scadenta', 'data_restituire', 'returnat'
        ]
...
class RecenzieSerializer(serializers.ModelSerializer):
    utilizator = serializers.HiddenField(default=serializers.CurrentUserDefault())
    utilizator_nume = serializers.CharField(source="utilizator.nume", read_only=True)
    utilizator_prenume = serializers.CharField(source="utilizator.prenume", read_only=True)
    utilizator_clasa = serializers.CharField(source="utilizator.clasa", read_only=True)
    autor_carte = serializers.CharField(source="carte.autor", read_only=True)
    ...
    class Meta:
        model = Recenzie
        fields = ['id', 'utilizator', 'utilizator_nume', 'utilizator_prenume', 'utilizator_clasa',
                  'carte', 'carte_titlu', 'autor_carte', 'rating', 'comentariu', 'data_postare', 'aprobat']
