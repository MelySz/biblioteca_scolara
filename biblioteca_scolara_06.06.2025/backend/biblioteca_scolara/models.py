...
class Utilizator(AbstractBaseUser, PermissionsMixin):
    ...
    data_suspendare = models.DateField(blank=True, null=True)
    zile_suspendare = models.IntegerField(default=3, blank=True, null=True)
    poza = models.ImageField(upload_to="poze_utilizatori/", blank=True, null=True, default="default/library.jpg")  
    domenii_interes = models.JSONField(default=list, blank=True) 
    
    def are_domenii_selectate(self):
        return bool(self.domenii_interes)

    def get_poza(self):
        """Returnează URL-ul pozei sau un placeholder"""
        if self.poza:
            return self.poza.url
        return "/media/default/library.jpg"
...
### Model pentru TokenResetareParola
class TokenResetareParola(models.Model):
    utilizator = models.ForeignKey(Utilizator, on_delete=models.CASCADE)
    token = models.UUIDField(default=uuid.uuid4, unique=True)
    data_expirare = models.DateTimeField()

    def este_valid(self):
        """Verifică dacă tokenul este încă valid."""
        return now() < self.data_expirare

    def __str__(self):
        return f"Token pentru {self.utilizator.email} - Expiră la {self.data_expirare}"
...
class Carte(models.Model):
    ...
    editura = models.CharField(max_length=255, blank=True, null=True)
    ...
    an_publicatie = models.IntegerField(
...
class Exemplar(models.Model):
    ...
    cod_unic = models.CharField(max_length=20, unique=True)  # număr de inventar
...
class Recenzie(models.Model):
    ...
    respins = models.BooleanField(default=False)
